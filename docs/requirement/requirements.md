...
| F-003 | メモ削除 | メモの削除（ゴミ箱機能は後日検討） | 必須 |
| F-004 | メモ一覧 | 保存済みメモの一覧表示 | 必須 |
| F-005 | メモ検索 | タイトル・本文でのローカル検索 | 必須 |
| F-006 | Markdownプレビュー | 編集中のMarkdownをリアルタイムプレビュー | 必須 |
...
| F-032 | （iOS）クイック起動 | クイックアクション/ウィジェット/ショートカット等から起動し、入力欄へフォーカス（ロック画面はOS制約あり） | 必須 |
...
- `local_updated_at` のタイムスタンプで最新を判定（TODO: 端末時計ズレ対策）
- 競合が発生した場合はユーザーに選択を促す
